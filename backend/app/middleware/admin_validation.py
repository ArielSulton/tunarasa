...
class AdminValidationService:
    ...
    def _load_blocked_keywords(self):
        ...
        try:
            ...
            default_keywords = {
                "spam", "scam", "fraud", "hack", "exploit", "vulnerability",
                "penipuan", "spam", "bohong", "palsu", "illegal"
            }
    ...
    async def _validate_security(
        ...
        try:
            ...
            operation_result = await self._validate_operation_permissions(request, operation)
            if operation_result:
    ...
    async def _validate_admin_session(self, request: Request) -> Optional[ValidationResult]:
    ...
    async def _validate_operation_permissions(
        ...
        high_risk_operations = {
            'delete_user', 'modify_system_settings', 'export_data',
            'reset_passwords', 'modify_permissions'
        }
        
        medium_risk_operations = {
            'update_user', 'view_sensitive_data', 'send_notifications'
        }
    ...
    async def _validate_performance(self, data: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if response_time > 15:
            results.append(ValidationResult(
                ...
                score=0.6,
                details={"response_time_limit": response_time, "recommended": "≤15s"}
            ))
        else:
            results.append(ValidationResult(
                category=ValidationCategory.PERFORMANCE,
    ...
    async def _validate_content(self, data: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if keywords:
            ...
            if len(keyword_list) > 100:
                results.append(ValidationResult(
                    ...
                    score=0.7,
                    details={"keyword_count": len(keyword_list), "recommended": "≤100"}
                ))
        ...
        if not data.get('enable_content_filtering', True):
            results.append(ValidationResult(
                ...
                score=0.5,
                details={"recommendation": "Enable content filtering for better security"}
            ))
    ...
    async def calculate_overall_validation_score(
        ...
        if not results:
            return 0.0, {"status": "no_validation", "message": "No validation performed"}
    ...
    async def update_blocked_keywords(self, keywords: str):
        ...
        try:
            ...
            logger.info(f"Updated blocked keywords: {len(self._blocked_keywords)} keywords")
