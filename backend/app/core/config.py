...
def test_database_connection():
    ...
    try:
        ...
        with engine.connect() as connection:
            # Test SSL connection - perbaiki cara query
            try:
                ssl_result = connection.execute(text("SELECT ssl_is_used()")).scalar()
            except:
                ssl_result = True  # Assume SSL is enabled if function not available
            ...
            print(f"✅ Database connection successful! (Query time: {query_time:.3f}s)")
            return True
