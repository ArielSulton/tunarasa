...
class LLMQualityAnalyzer:
    ...
    def _analyze_completeness(self, qa_example: QAExample) -> float:
        ...
        completeness_indicators = [
            ...
            "karena" in answer or "because" in answer or "sebab" in answer,  # Ada penjelasan
    ...
    def _analyze_clarity(self, qa_example: QAExample) -> float:
        ...
        repetition_score = 1.0 if max_repetition <= 3 else max(0.0, 1.0 - (max_repetition - 3) * 0.2)
        clarity_indicators.append(repetition_score)
        
        # 3. Struktur yang jelas
        structure_words = ["pertama", "kedua", "kemudian", "selanjutnya", "finally", "first", "second", "then"]
...
class LLMRecommendationService:
    ...
    async def analyze_qa_batch(self, qa_examples: List[QAExample]) -> List[LLMRecommendation]:
    ...
    async def _generate_recommendations(self, quality_analyses: List[Dict]) -> List[LLMRecommendation]:
        ...
        if overall_metrics["avg_context_utilization"] < 0.7:
            recommendations.append(self._create_context_recommendation(quality_analyses))
        
        # 3. Rekomendasi berdasarkan clarity
        if overall_metrics["avg_clarity_score"] < 0.7:
            recommendations.append(self._create_clarity_recommendation(quality_analyses))
        
        # 4. Rekomendasi berdasarkan response time
        if overall_metrics["avg_efficiency_score"] < 0.8:
            recommendations.append(self._create_efficiency_recommendation(quality_analyses))
        
        # 5. Rekomendasi berdasarkan completeness
        if overall_metrics["avg_completeness_score"] < 0.7:
            recommendations.append(self._create_completeness_recommendation(quality_analyses))
        
        # Filter dan ranking rekomendasi
        valid_recommendations = [r for r in recommendations if r is not None]
        return sorted(valid_recommendations, key=lambda x: (x.priority.value, -x.confidence))
    
    def _calculate_overall_metrics(self, quality_analyses: List[Dict]) -> Dict[str, float]:
        ...
        metrics = {}
        metric_keys = [
            "length_score", "context_utilization", "completeness_score",
            "clarity_score", "relevance_score", "efficiency_score", "overall_quality"
        ]
    ...
    def _create_length_recommendation(self, quality_analyses: List[Dict]) -> Optional[LLMRecommendation]:
        """Buat rekomendasi untuk panjang respons"""
        template = self.recommendation_templates["short_response"]
        
        # Analisis detail
        short_responses = [qa for qa in quality_analyses if qa["quality"]["length_score"] < 0.6]
    ...
    def _create_context_recommendation(self, quality_analyses: List[Dict]) -> Optional[LLMRecommendation]:
        """Buat rekomendasi untuk penggunaan konteks"""
        template = self.recommendation_templates["low_context_usage"]
        
        poor_context_usage = [qa for qa in quality_analyses if qa["quality"]["context_utilization"] < 0.7]
    ...
    def _create_clarity_recommendation(self, quality_analyses: List[Dict]) -> Optional[LLMRecommendation]:
        """Buat rekomendasi untuk kejelasan bahasa"""
        template = self.recommendation_templates["unclear_language"]
        
        unclear_responses = [qa for qa in quality_analyses if qa["quality"]["clarity_score"] < 0.7]
    ...
    def _create_efficiency_recommendation(self, quality_analyses: List[Dict]) -> Optional[LLMRecommendation]:
        """Buat rekomendasi untuk efisiensi respons"""
        template = self.recommendation_templates["slow_response"]
        
        slow_responses = [qa for qa in quality_analyses if qa["quality"]["efficiency_score"] < 0.8]
    ...
    def _create_completeness_recommendation(self, quality_analyses: List[Dict]) -> Optional[LLMRecommendation]:
        """Buat rekomendasi untuk kelengkapan jawaban"""
        template = self.recommendation_templates["incomplete_answer"]
        
        incomplete_responses = [qa for qa in quality_analyses if qa["quality"]["completeness_score"] < 0.7]
    ...
    async def get_recommendations_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": [r.to_dict() for r in recommendations],
                "key_metrics": {
                    "avg_response_time": np.mean([qa.response_time for qa in recent_qa]),
                    "avg_confidence": np.mean([qa.confidence for qa in recent_qa]),
                    "quality_distribution": {
                        "excellent": len([qa for qa in quality_analyses if qa["quality"]["overall_quality"] >= 0.8]),
                        "good": len([qa for qa in quality_analyses if 0.6 <= qa["quality"]["overall_quality"] < 0.8]),
                        "needs_improvement": len([qa for qa in quality_analyses if qa["quality"]["overall_quality"] < 0.6])
                    }
                },
                "generated_at": datetime.utcnow().isoformat()
            }
...
async def analyze_and_recommend(
    ...
    return {
        ...
        "recommendations": [r.to_dict() for r in recommendations],
        "summary": {
            "high_priority": len([r for r in recommendations if r.priority == PriorityLevel.HIGH]),
            "medium_priority": len([r for r in recommendations if r.priority == PriorityLevel.MEDIUM]),
            "low_priority": len([r for r in recommendations if r.priority == PriorityLevel.LOW])
        }
    }