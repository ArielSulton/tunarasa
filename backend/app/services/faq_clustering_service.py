    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            response = self.pc.inference.embed(
                model=self.model, inputs=texts, parameters={"input_type": "query"}
            )
...
class SimplifiedFAQClusteringService:
    ...
    def _initialize_vectorstore(self):
        ...
        try:
            if settings.PINECONE_API_KEY and self.embeddings:
                ...
                # Note: This service uses Pinecone client directly, not LangChain integration
                # For now, disable vector store functionality until proper LangChain integration
                logger.warning(
                    "FAQ Clustering Service: Vector store initialization disabled - needs LangChain integration"
                )
                self.vectorstore = None
    ...
    def get_representative_questions(
        self, questions: List[str], embeddings: np.ndarray, clusters: np.ndarray, kmeans
        ...
        for cluster_id in range(kmeans.n_clusters):
            ...
            avg_similarity = (
                float(np.mean(cluster_similarities)) if cluster_similarities else 0
            )
            ...
            logger.info(
                f"Cluster {cluster_id}: {len(cluster_questions)} questions, "
    ...
    def get_cluster_keywords(
        self, questions: List[str], clusters: np.ndarray, n_clusters: int
    ) -> Dict[int, List[str]]:
        ...
        for cluster_id in range(n_clusters):
            ...
            if cluster_questions:
                ...
                representative = cluster_questions[
                    0
                ]  # or use the actual representative
    ...
    def search_similar_questions(
        ...
        try:
            ...
            query_embedding = self.embeddings.embed_query(
                self.simple_preprocess_text(query)
            )

            # Search in Pinecone with max_results to filter later
            results = self.index.query(
                vector=query_embedding, top_k=max_results, include_metadata=True
            )
            ...
            if len(filtered_results) == 0 and min_results > 0:
                logger.warning(
                    f"No results above threshold {similarity_threshold}, returning top {min_results}"
                )
                for match in results["matches"][:min_results]:
                    filtered_results.append(
                        {
                            "id": match["id"],
                            "score": match["score"],
            ...
            logger.info(
                f"Found {len(filtered_results)} results above threshold {similarity_threshold}"
            )
    ...
    def search_with_adaptive_threshold(
        ...
        while current_threshold >= min_threshold:
            ...
            if len(results) >= target_count:
                logger.info(
                    f"Found {len(results)} results with threshold {current_threshold}"
                )
            ...
            if len(results) > 0 and current_threshold <= min_threshold:
                logger.info(
                    f"Returning {len(results)} results at minimum threshold {min_threshold}"
                )
                return results
        ...
        logger.warning(
            f"Could not find {target_count} results above {min_threshold}, returning best matches"
        )
    ...
    def get_similarity_statistics(
        self, query: str, max_samples: int = 50
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            results = self.index.query(
                vector=query_embedding, top_k=max_samples, include_metadata=True
            )
    ...
    def search_with_confidence_levels(
        ...
        for result in all_results:
            ...
            result["confidence_level"] = (
                ...
                if score >= high_threshold
                    ...
                    else "low" if score >= low_threshold else "very_low"
                )
            )
    ...
    def store_questions_in_vectorstore(
        self, questions: List[str], metadata: Optional[List[Dict]] = None
    ):
