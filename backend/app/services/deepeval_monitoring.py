...
class DeepEvalMonitoringService:
        ...
        try:
            self.metrics = {
                ...
                EvaluationCategory.HALLUCINATION: HallucinationMetric(
                    threshold=QualityThreshold.ACCEPTABLE.value,
                    model=self.model
                ),
    ...
    async def _evaluate_metric(
        ...
        try:
            ...
            passed = metric.is_successful() if hasattr(metric, 'is_successful') else score >= threshold
    ...
    async def _evaluate_performance(self, conversation: LLMConversation) -> EvaluationResult:
    ...
    async def _evaluate_accuracy(self, conversation: LLMConversation) -> Optional[EvaluationResult]:
    ...
    async def _update_performance_metrics(self, results: List[EvaluationResult]):
        ...
        try:
            ...
            for result in results:
                ...
                metrics["pass_rate"] = len([s for s in scores if s >= result.threshold]) / len(scores)
    ...
    async def get_evaluation_summary(
        ...
        try:
            ...
            for category in EvaluationCategory:
                category_results = [r for r in recent_evaluations if r.category == category]
                if category_results:
                    scores = [r.score for r in category_results]
                    category_stats[category.value] = {
                        "count": len(category_results),
                        "average_score": sum(scores) / len(scores),
                        "pass_rate": len([r for r in category_results if r.passed]) / len(category_results),
            ...
            overall_pass_rate = len([r for r in recent_evaluations if r.passed]) / len(recent_evaluations)
    ...
    async def generate_quality_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if quality_scores:
                overall_avg = sum(q["average_score"] for q in quality_scores.values()) / len(quality_scores)
                overall_pass_rate = sum(q["pass_rate"] for q in quality_scores.values()) / len(quality_scores)
            else:
                overall_avg = 0.0
