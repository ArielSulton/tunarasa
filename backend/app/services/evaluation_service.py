...
class EvaluationService:
    ...
    def __init__(self):
        self.metrics_enabled = (
            DEEPEVAL_AVAILABLE and settings.ENVIRONMENT != "production"
        )
        ...
        if self.metrics_enabled:
            try:
                ...
                self.answer_relevancy = AnswerRelevancyMetric(
                    threshold=0.7, model=groq_model
                )
        ...
        logger.info(
            f"Evaluation service initialized. Metrics enabled: {self.metrics_enabled}"
        )
        ...
        try:
            ...
            try:
                ...
                evaluation_results["answer_relevancy"] = {
                    ...
                    "passed": self.answer_relevancy.score
                    >= self.answer_relevancy.threshold,
                    "reason": getattr(
                        self.answer_relevancy, "reason", "No specific reason"
                    ),
                }
            except Exception as e:
            ...
            try:
                ...
                evaluation_results["faithfulness"] = {
                    ...
                    "reason": getattr(
                        self.faithfulness, "reason", "No specific reason"
                    ),
                }
            except Exception as e:
    ...
    def _create_error_evaluation(
        self, question: str, answer: str, error: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {},
            "recommendations": [
                "Evaluation failed - please check system configuration"
            ],
            "quality_grade": "F",
        }

    def _generate_recommendations(
        self, metrics: Dict, overall_score: float
    ) -> List[str]:
        ...
        if overall_score < 0.5:
            recommendations.append(
                "Overall quality is low - consider improving answer relevance and accuracy"
            )
        elif overall_score < 0.7:
            recommendations.append(
                "Moderate quality - room for improvement in specific areas"
            )
        else:
            ...
            if isinstance(result, dict) and "score" in result:
                if not result.get("passed", False):
                    if metric_name == "answer_relevancy":
                        recommendations.append(
                            "Improve answer relevance to the question"
                        )
                    elif metric_name == "faithfulness":
                        recommendations.append(
                            "Ensure answer is faithful to the source context"
                        )
                    elif metric_name == "contextual_precision":
                        recommendations.append(
                            "Improve contextual precision and accuracy"
                        )
    ...
    def get_evaluation_summary(self, time_range: str = "24h") -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_score": 0.82,
                "quality_distribution": {"A": 45, "B": 60, "C": 30, "D": 10, "F": 5},
                "common_issues": [
