...
class QRCodeService:
    ...
    def __init__(self):
        self.base_url = "http://localhost:8000"  # Production URL
    ...
    def create_summary_document(
        ...
        try:
            if format_type == "json":
                ...
                return self._create_html_summary(conversation_data, messages, summary_text, title)
            else:
                return self._create_text_summary(conversation_data, messages, summary_text, title)
                
        except Exception as e:
            logger.error(f"Failed to create summary document: {e}")
            raise
    
    def _create_text_summary(self, conversation_data: Dict, messages: list, summary: str, title_text: str) -> str:
...
"""
    ...
    def _create_html_summary(self, conversation_data: Dict, messages: list, summary: str, title_text: str) -> str:
        """Create HTML summary"""
        ...
        formatted_content = "".join([f"<p>{message.strip()}</p>" for message in formatted_messages])
