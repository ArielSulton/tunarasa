...
class PineconeVectorService:
    ...
    async def ingest_document(
        ...
        try:
            ...
            if 'doc_metadata' in locals():
                ...
                doc_metadata.processing_time = (datetime.utcnow() - start_time).total_seconds()
    ...
    async def _embed_and_store_chunks(
        ...
        try:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                for j, (chunk, embedding) in enumerate(zip(batch_chunks, embeddings)):
                    ...
                    pinecone_metadata = {
                        ...
                        "content": chunk.page_content[:1000],  # First 1000 chars for search
            ...
            logger.info(f"Stored {len(all_vectors)} vectors for document {doc_metadata.document_id}")
    ...
    async def batch_ingest_documents(
        ...
        async def process_single_document(file_path: str, metadata: Optional[Dict[str, Any]]):
            """Process single document with semaphore"""
            async with semaphore:
                try:
                    result = await self.ingest_document(file_path, metadata=metadata)
    ...
    async def delete_document(self, document_id: str) -> bool:
        ...
        try:
            ...
            if document_id in self.document_metadata_cache:
                self.document_metadata_cache[document_id].processing_status = ProcessingStatus.DELETED
    ...
    async def update_document(
        ...
        try:
            ...
            # Ingest new version
            new_metadata = await self.ingest_document(
        ...
        try:
            ...
            if search_results.matches:
                ...
                doc_metadata = DocumentMetadata(
                    ...
                    upload_timestamp=datetime.fromisoformat(metadata.get('upload_timestamp', datetime.utcnow().isoformat())),
    ...
    async def list_documents(
        ...
        # Apply status filter
        if status_filter:
            documents = [doc for doc in documents if doc.processing_status == status_filter]
