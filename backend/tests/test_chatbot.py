...
def test_basic_chatbot_response():
    ...
    for keyword in expected_keywords:
        assert (
            keyword.lower() in actual_output.lower()
...
def test_chatbot_response_quality():
    ...
    for i, case in enumerate(test_cases):
        ...
        assert (
            min_len <= len(output) <= max_len
        ), f"Case {i+1}: Response length {len(output)} not in range {case['expected_length_range']}"

        # Content check
        for phrase in case["must_contain"]:
            assert (
                phrase.lower() in output.lower()
            ), f"Case {i+1}: Required phrase '{phrase}' not found"
...
def test_chatbot_error_handling():
    ...
    assert (
        "sorry" in actual_response.lower() or "error" in actual_response.lower()
    ), "Error response should be apologetic"
...
def test_response_time_simulation():
    ...
    _response = "You have 30 days to get a full refund at no extra cost."
